...
// Tolerance-based comparison to handle floating-point inaccuracies
// bool configurationsEqual(double* v1, double* v2, int size, double tolerance = 1e-5) {
//     for (int i = 0; i < size; ++i) {
...
// Corrected Euclidean distance calculation
...
// Find all nodes within a specified rewiring radius
...
static void plannerRRTStar(
    ...
    int *planlength) 
{
    int maxIterations = 500000;         // Maximum number of iterations
    ...
    for (int iter = 0; iter < maxIterations; ++iter) {
        ...
        if (IsValidArmConfiguration(expandedNode->config, numofDOFs, map, x_size, y_size)) {
            ...
            // Check if the goal has been reached within a small tolerance
            if (calculateRRTStarDistance(expandedNode->config, armgoal_anglesV_rad, numofDOFs) < expansionStep) {
                ...
                // Ensure start and goal configurations explicitly in path
                if (!configurationsEqual(path.back()->config, armstart_anglesV_rad, numofDOFs)) {
...
// Interpolates configurations to ensure a continuous collision-free path
...
// Connects nodes within a specified radius, ensuring collision-free paths
...
// Samples collision-free configurations for PRM
std::vector<PRMNode*> sampleConfigurations(int numSamples, int numOfDOFs, double* map, int xSize, int ySize) {
    ...
    for (int i = 0; i < numSamples; ++i) {
        ...
        if (IsValidArmConfiguration(config, numOfDOFs, map, xSize, ySize)) {
            ...
            delete[] config;  // Clean up memory for invalid configurations
        }
    }
...
// A* search to find the shortest path in PRM
